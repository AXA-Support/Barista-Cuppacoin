...
const LibraryPage = () => {
  ...
  // Calculate counts for ALL filters based on allTrainings
  ...
  // Calculate pagination based on filtered trainings
  const totalItems = filteredTrainings.length;
  const totalPages = Math.ceil(totalItems / itemsPerPage);

  // Fix: When total pages change and current page is greater than total pages, go to last page
  useEffect(() => {
  ...
  //   try {
  //     const response = await trainingAPI.download(training.id);
  ...
  //     try {
  //       const fileUrl = trainingAPI.getFileUrl(training.file_path);
  //       const a = document.createElement('a');
  //       a.href = fileUrl;
  //       a.download = training.file_name || training.title;
  //       a.target = '_blank';
 ...
 //   console.log("Downloading:", training);
    ...
    // Direct download using the route URL (WITHOUT /api)
    const a = document.createElement('a');
